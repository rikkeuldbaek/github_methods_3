...
#plot 
plt.figure()
plt.plot(avg_rep_PAS_1)
plt.plot(avg_rep_PAS_2)
plt.plot(avg_rep_PAS_3)
plt.plot(avg_rep_PAS_4)
plt.axvline()
plt.axhline()
plt.xlabel(" time")
plt.ylabel("magnetic field")
...
#data_1_2= np.concatenate((data[PAS_1,:,:],data[PAS_2,:,:]), axis=1) #HAR 4 DIM
...
y_1= np.where(y == 1)
y_2= np.where(y == 2)
y_1_1= np.concatenate((y_1, y_2), axis = 1)
y_1_1.shape
y_1_2= y_1_1[0]
y_1_2.shape


#ELLER skal jeg lave en target variable med 1's and 0's ? 
nums = np.ones(214)
nums[:99] = 0
...
regressor = LogisticRegression(penalty ='none').fit(scaled_X_1_2, y_1_2) #renaming stuff for simplicity
logistic_hat = regressor.predict(scaled_X_1_2) #a bunch of 4's
len(logistic_hat)

#Use the `score` method of `LogisticRegression` to find out how many labels were classified correctly. 
regressor.score(X_1_2, y_1_2) # 0.004672897196261682, 

#slope and intercept? 
intercept = regressor.intercept_;print(intercept) #PRINTING INTERCEPT
slope = regressor.coef_;print(slope) # printing SLOPE

```
  
vi. Now apply the _L1_ penalty instead - how many of the coefficients (`.coef_`) are non-zero after this?  
```{python}
# Do a standard `LogisticRegression` - this time use penalty 'L1'
from sklearn.linear_model import LogisticRegression #importing 
regressor_L1 = LogisticRegression(penalty ='l1', solver= 'saga').fit(scaled_X_1_2, y_1_2) #renaming stuff for simplicity
logistic_hat_1 = regressor.predict(scaled_X_1_2) #a bunch of 4's
len(logistic_hat_1)
