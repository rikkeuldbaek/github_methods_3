...
## 4 my own sake, an understanding of arrays
```{python}
import numpy as np
x = np.arange(24).reshape(4,3,2)
#se Mina's billede for visuel forståelse
```


```{r}
pacman::p_load(png, patchwork)
![Arras visually.](/Users/rikkeuldbaek/Desktop/Cognitive Science/3rd semester/Methods 3/github_methods_3/week_08/arrays_pic.png)
...
laus svar: 
antagelse alle variabler er uafhængige, men det er de ikke
men vi har correlation alle muligsteder så de picker ikke up independent signals

```{python}
#     iii. Create the sensor covariance matrix $\Sigma_{XX}$: $$\Sigma_{XX} = \frac 1 N \sum_{i=1}^N XX^T$$ $N$ is the number of repetitions and $X$ has $s$ rows and $t$ columns (sensors and time), thus the shape is $X_{s\times t}$. Do the sensors pick up independent signals? (Use plt.imshow to plot the sensor covariance matrix)  
#1) make X
data.shape #102 sensors (rows)
sensors = data[:,2]
len(sensors)
times.shape #251 times (columns)

#an example for my own sake
try_1= data[12,:,:]
len(try_1)
try_1.shape #102 rows and 251 column
...
print(maxiking) # sensor nr 73 ved repetition 112 har maksimale magnetic field 

print(avr_rep[73,112]) #2.7886216843591933e-13 # is the same as below (SAME)
np.amax(avr_rep) #2.7886216843591933e-13 # den maximum value in the average (SAME)
```
...
```{python}
#Make 4 averages, one for each PAS rating
#1) extract each pas rating individually
pas_1 = [x for x in y if x == 1]
pas_2 = [x for x in y if x == 2]
pas_3 = [x for x in y if x == 3]
pas_4 = [x for x in y if x == 4]

#how many pas ratings for each PAS? 
print(data[pas_1,:,:].shape) #99 pas-ratings 1 (in all data)
print(data[pas_2,:,:].shape) #115 pas-ratings 2 (in all data)
print(data[pas_3,:,:].shape) #208 pas-ratings 2 (in all data)
print(data[pas_4,:,:].shape) #260 pas-ratings 4 (in all data)

# Pas ratings for sensor 73
data[pas_1,73,:] #all pas rating 1's for sensor 73
data[pas_2,73,:] #all pas rating 2's for sensor 73
data[pas_3,73,:] #all pas rating 3's for sensor 73
data[pas_4,73,:] #all pas rating 4's for sensor 73


#2) find det sted hvor alle repetitions står samme sted som deres pas rating
avr_rep_pas1= np.mean(data[pas_1,73,:], axis = 0)
avr_rep_pas2= np.mean(data[pas_2,73,:], axis = 0)
avr_rep_pas3= np.mean(data[pas_3,73,:], axis = 0)
avr_rep_pas4= np.mean(data[pas_4,73,:], axis = 0)
...
np.mean(data, axis=0)
avr_rep.shape #inspect

#Plot the four time courses (one for each PAS rating) for the sensor found in Exercise 1.1.v
#plot 
plt.figure()
plt.plot(avr_rep_pas1)
plt.plot(avr_rep_pas2)
plt.plot(avr_rep_pas3)
plt.plot(avr_rep_pas4)
plt.axvline()
plt.axhline()
plt.xlabel(" time")
plt.ylabel("magnetic field")
