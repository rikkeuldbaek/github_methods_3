...
After applying L1 penalty 217 of my coefficients are non-zero which makes sense since .... 
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*  ???
...
#rehape X_1_2_equal from 3D to 2D
...
cv = StratifiedKFold() #abbriviation for simplicity

scaled_X_1_2_equal.shape # our X (standardized)
y_1_2_equal.shape # our y

scores = cross_val_score(logR_standard, scaled_X_1_2_equal, y_1_2_equal, cv=5) #cv = 5 is 5 folds
...
i picked the last model that resulted in the largest score of 0.4805128205128205

and the largest score of the cross val loop = 0.5752564102564103

```{python}
###############  L2 with C=1e-5 (0.00001) is the best model 
cv_scores= [] #make an empty list
log_loop= LogisticRegression(C=1e-5, penalty ='l2', solver = "liblinear", random_state= 1) #best model from before defined before the loop :)
...
np.argmax(cv_scores) # index for maksimale punkt
...
plt.axvline((times[152]), color= "black") #Highlight the time point with the maximal magnetic field in the average
