...
#plot 
plt.figure()
plt.plot(times, avg_rep_PAS_1.T)
plt.plot(times, avg_rep_PAS_2.T)
plt.plot(times, avg_rep_PAS_3.T)
plt.plot(times, avg_rep_PAS_4.T)
...
logR = LogisticRegression(penalty ='none')#renaming stuff for simplicity
logR.fit(scaled_X_1_2, y_1_2)
...
After applying L1 penalty 4448 of my coefficients are non-zero which makes sense since .... 
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*  ???
...
from sklearn.linear_model import LogisticRegression #importing 
logR_L1 = LogisticRegression(penalty ='l1', tol = 0.01, solver = "liblinear", random_state= 1) #renaming stuff for simplicity
logR_L1.fit(scaled_X_1_2, y_1_2) 
...
nuller = np.where(logR.coef_ >0)
type(nuller)

nuller_ = nuller.astype('int32')
...
```{python}
#my first take, makes little sense? but equal as above
X_reduced = logR_L1.coef_[logR_L1.coef_ !=0]
len(X_reduced) # 4448

#Create a new reduced $X$ that only includes the non-zero coefficients
X_reduced = np.where(logR_L1.coef_[0] !=0)
X_reduced = np.where(logR_L1.coef_ !=0)
reduced_data=np.squeeze(scaled_X_1_2[:,scaled_X_1_2])
cov_mat = np.cov(reduced.data)
...
######NEW TAKE
cov_mat = []
n= np.sum(logR_L1.coef_ !=0)

for i in range(n):
  cov_mat.append(X_reduced[:,i] @ X_reduced[:,i].T) 
...
```{python}
# imports :))
from sklearn.model_selection import StratifiedKFold #importing stuff 1
from sklearn.model_selection import cross_val_score #importing stuff 2
```
