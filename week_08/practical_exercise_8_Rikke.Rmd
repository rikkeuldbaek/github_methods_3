...
We see some covariance sporadically distributed in the sensor covariance matrix, this suggests that they do not really pick up independent signals. This may be, since we assume that all variables are independent, which they are not.  *!!!!!!!* ???
...
In the plot above we see each single repetition of visual stimuli (682 repetitions) for sensor 73. The individual repetitions contains a lot of noise (I assume that the noise is the squiggles). The response found in the average plot (2.1 v (Plot the magnetic field (based on the average))) where we see a large peak around time 112, is also visualized in the plot above where we see a little concave curve around time 112 as well.I would say the plot in exercise 2.1 v (Plot the magnetic field (based on the average)) is more informative since the signals are based on an average per sensor, which makes it easier to distinguish the between them. 
...
We could suspect that we are overfitting because we do not penalize. 
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*  ???
...
import matplotlib.pyplot as plt
plt.figure()
plt.imshow(covariance_matrix_1)
plt.colorbar()
plt.show()
```
...
*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*  ???
Not really sure I answered the above question 
...
plt.axvline((times[152]), color= "black") #Highlight of time point where classification is best 
...
The time point where the classification using L1 with C=1e-1 is best is at time point 36, as visualized in the plot. . 
```{python}
###############  L1 with C=1e-1 (0.1) 
...
plt.axvline((times[36]), color= "black") #Highlight of time point where classification is best 
plt.axhline(0.5) #chance level of 0.5, since its binary
plt.xlabel(" time")
plt.ylabel("classification score")
...
When fitting logistic regression on each time sample from PAS 1 and PAS 4 ratings with the model using L1 regularization with C=1e-1 (0.1), the classification is best at time point 78. The chance level for this analysis is 0.5 as we have a binary outcome. 
...
scaled_X_4.shape # my X
y_4.shape # my y

svm = SVC(kernel='linear', C=1, random_state=7, gamma=1e1)

scores_svm = cross_val_score(svm, scaled_X_4, y_4, cv=cv)
print(np.mean(scores_svm)) #0.2928164556962025
```
