---
title: "Assignment 3, Methods 3, 2021, autumn semester"
author: "Rikke Uldbæk"
...
```{python, message=FALSE, warning=FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
#Make an average over the repetition dimension using `np.mean` 
...
```{python, message=FALSE, warning=FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
#Plot the magnetic field for each of the repetitions (a line for each)
#for the sensor that has the maximal magnetic field (sensor 73)
plt.figure()
plt.plot(times, data[:,73,:].T)
plt.axvline((times[112]), color= "black") #maximal magnetic field in the average
plt.axhline()
plt.xlabel(" time")
plt.ylabel("magnetic field")
plt.title("magnetic field (based on the average)")
...
for i in range(len(y)):
    if y[i] == 1:
        y_1_2.append(1) #append real numbers and not indices
    if y[i] == 2:
        y_1_2.append(2) #append real numbers and not indices
```
...
#Use the `score` method of `LogisticRegression` 
# to find out how many labels were classified correctly. 
...
from sklearn.linear_model import LogisticRegression #importing 
logR_L1 = LogisticRegression(penalty ='l1', solver = "liblinear", random_state= 1)
...
It looks like there is bit more positive covariance () in this covariance matrix compared to the plot from 1.1.iii. But honestly, it's a little hard to compare, when we have two matrices with variables on different scales. 
...
plt.title("covariance of the non-zero features")
...
#plotting the covariance matrix (from earlier)
plt.figure()
plt.imshow(covariance_matrix,cmap = "hot")
plt.colorbar()
plt.xlabel("sensors")
...
#rehape data_1_2_equal from 3D to X_1_2_equal 2D
...
cv = StratifiedKFold(n_splits=5) #abbreviation for simplicity

scaled_X_1_2_equal.shape # our X (standardized)
y_1_2_equal.shape # our y

scores = cross_val_score(logR_standard, scaled_X_1_2_equal, y_1_2_equal, cv=cv)
...
logR_L2_1 = LogisticRegression(C=1e5, penalty ='l2', solver = "liblinear", random_state= 1)
...
###############  L2 with C=1e1 (10)
logR_L2_2 = LogisticRegression(C=1e1, penalty ='l2', solver = "liblinear", random_state= 1)
logR_L2_2.fit(scaled_X_1_2_equal, y_1_2_equal) 
...
logR_L2_3 = LogisticRegression(C=1e-5, penalty ='l2', solver = "liblinear", random_state= 1) 
logR_L2_3.fit(scaled_X_1_2_equal, y_1_2_equal)

#Cross val score
scores_L2_3 = cross_val_score(logR_L2_3, scaled_X_1_2_equal, y_1_2_equal, cv=5) 
...
log_loop= LogisticRegression(C=1e-5, penalty ='l2', solver = "liblinear", random_state= 1) 
#best model from above defined before the loop

#logistic regression on 1 time sample
for i in range(251):
  transformed_data = scaler.fit_transform(data_1_2_equal[:,:,i])
  log_loop.fit(transformed_data, y_1_2_equal) 
  scorez= cross_val_score(log_loop, transformed_data, y_1_2_equal, cv=5)
  cv_scores.append(np.mean(scorez))
```
...
#plot time on the x-axis and classification score on the y-axis with line at the chance level
plt.figure()
...
log_loop_l1= LogisticRegression(C=1e-1, penalty ='l1', solver = "liblinear", random_state= 1) 
#best model from above defined before the loop :)

#logistic regression on 1 time sample
for i in range(251):
  transformed_data_l1 = scaler.fit_transform(data_1_2_equal[:,:,i]) 
  # transform the data first
  log_loop_l1.fit(transformed_data_l1, y_1_2_equal) 
  #loop and fit through the transformed data
  scorez_l1= cross_val_score(log_loop_l1, transformed_data_l1, y_1_2_equal, cv=5) 
  #calculate cross validation scores
  cv_scores_l1.append(np.mean(scorez_l1)) 
...
```{python, message=FALSE, warning=FALSE,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
# create a data set and a target vector that only 
# contains PAS responses 1 and 4 (`data_1_4` and `y_1_4`)
...
for i in range(len(y)):
    if y[i] == 1:
        y_1_4.append(1) #append real numbers and not indices 
        #(if u put in 'i' then they append the number)
    if y[i] == 2:
        y_1_4.append(4) #append real numbers and not indices 
        #(if u put in 'i' then they append the number)
...
log_loop_1_4= LogisticRegression(C=1e-1, penalty ='l1', solver = "liblinear", random_state= 1) 
#best model from above defined before the loop :)

#logistic regression on 1 time sample
for i in range(251):
  transformed_data_1_4 = scaler.fit_transform(data_1_4_equal[:,:,i]) 
  # transform the data first
  log_loop_1_4.fit(transformed_data_1_4, y_1_4_equal) 
  #loop and fit through the transformed data
  scorez_1_4= cross_val_score(log_loop_1_4, transformed_data_1_4, y_1_4_equal, cv=5) 
  #calculate cross validation scores
  cv_scores_1_4.append(np.mean(scorez_1_4)) 
  #append to the list 'cv_scores'
```
...
#logistic regression on 1 time sample
for i in range(251):
  transformed_data_radial = scaler.fit_transform(data_4[:,:,i]) 
  # transform the data first
  radial_svm.fit(transformed_data_radial, y_4) 
  #loop and fit through the transformed data
  scorez_4= cross_val_score(radial_svm, transformed_data_radial, y_4, cv=5) 
  #calculate cross validation scores
  radial_cv_scores.append(np.mean(scorez_4)) 
  #append to the list 'cv_scores'
```
