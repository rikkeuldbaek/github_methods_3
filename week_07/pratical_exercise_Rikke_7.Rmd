...
We can express regression as $y = f(x) + \epsilon$ with $E[\epsilon] = 0$ (expected error of 0) and $var(\epsilon) = \sigma^2$ ($E$ means expected value)  
...
*Så mean squared error består af bias, variance or irriducible error, se overstående formel* ^ 
...
x = np.arange(0, 6.1, 0.1) # lav x's


y = true_function(x) #generer y ud fra true_function using x
...
# iii. plot the true function and the generated points
plt.figure()
plt.plot(x, y, "g") #function generated y's
plt.plot(x, y_noisy, "ro")
plt.title("plot4lyfe")
plt.legend(["yo dawg", "hehe"])
plt.show()
...
2) Fit a linear regression using `LinearRegression` from `sklearn.linear_model` based on $y_{noisy}$ and $x$ (see code chunk below associated with Exercise 1.2)  
    ...
    - The X_quadratic amounts to the 
...
from sklearn.linear_model import LinearRegression #import things
x=x.reshape(61,1) #vi skal lige reshape x fordi den er lidt mærkelig, husk assign til x, ellers er det fucked

lin_reg_model = LinearRegression().fit(x, y_noisy) #make da model

r.sq = lin_reg_model.score(x, y_noisy) #making r^2 


#i. plot the fitted line (see the `.intercept_` and `.coef_` attributes of the `regressor` object) on top of the plot (from 1.1.iii)

intercept = lin_reg_model.intercept_ #PRINTING INTERCEPT
slope = lin_reg_model.coef_ # printing SLOPE

predict_y = lin_reg_model.predict(x) #lav predicted values ud fra model (i.e plot intercept og slope, eller i hvertfald de værdier som disse betaer genererer)

plt.figure()
plt.plot(x, y, "g") #function generated y's
plt.plot(x, y_noisy, "ro")
plt.plot(x, predict_y, "b") #linear regression generated y's
plt.title("plot4lyfe")
plt.legend(["true function", "generated y's (noisy y's)", "linear model"])
plt.show()
...
X_quadratic = quadratic.fit_transform(x.reshape(-1, 1)) # X matrix
regressor = LinearRegression() #renaming stuff for simplicity, men det er underordnet 
quadratic_model = regressor.fit(X_quadratic, y_noisy)

y_quadratic_hat = quadratic_model.predict(X_quadratic)
...
plt.plot(x, y_noisy, "ro")
plt.plot(x, predict_y, "b") #linear regression generated y's
plt.plot(x, y_quadratic_hat, "black") #quadratic model generated y's
plt.title("plot4lyfe")
plt.legend(["true function", "generated y's (noisy y's)", "linear line", "quadratic line"])
plt.show()
...
X_fifth_order = fifth_order.fit_transform(x.reshape(-1, 1)) # X matrix
regressor = LinearRegression() #renaming stuff for simplicity, men det er underordnet 
fifth_order_model = regressor.fit(X_fifth_order, y_noisy)

y_fifth_order_hat = fifth_order_model.predict(X_fifth_order)

#plot of fifth order model
plt.figure()
plt.plot(x, y, "g") #function generated y's
plt.plot(x, y_noisy, "ro")
plt.plot(x, predict_y, "b") #linear regression generated y's
plt.plot(x, y_quadratic_hat, "black") #quadratic model generated y's
plt.plot(x, y_fifth_order_hat, "pink") #fifth order model generated y's
plt.title("plot4lyfe")
plt.legend(["true function", "generated y's (noisy y's)", "linear line", "quadratic line", "5th order"])
plt.show()
...
```{python, eval=FALSE}
# simulate 100 samples each sample size `len(x)` with $\sigma=5$ normally distributed noise added on top of the true function
y_sample_list = [y + np.random.normal(loc = 0, scale= 5, size = len(x)) for i in range(100)]
...
```{python, eval=FALSE}
#predict fits for the 3 models
from sklearn.linear_model import LinearRegression #import things

regressor = LinearRegression() # make the regressor (lm)
linear_list= [] #empty list

for i in range(100):
  linear_fits = regressor.fit(x, y_sample_list) #make da model 100 times
  predicted_linear_ys = linear_fits.predict(x)
