...
pacman::p_load(dplyr, tidyverse, patchwork)
...
###my own try
```{python}
#laver lige en python chunk lålå
name= "Rikke"
print(name)
```
...
summary(model) #per increment of weight, miles per gallon(mgp) goes down by -5.3445
```
...
# Then we can derive our residuals
n <- length(Y) # number of observations
p <- ncol(X) - 1 # number of covariates
i <- diag(1, nrow = n, ncol = n) # identity matrix
h <- X %*% X2 %*% t(X) # hat matrix of X
epsilon <- (i - h) %*% Y # Model residuals
print(data.table::data.table(epsilon)[, .(Residuals = V1)])

####OR generate errors like this (this is for my own sake, ignore please)
err<- tibble(res_extract_model=residuals(model))
df <- tibble(yhat=yhat, y=mtcars$mpg, errors = (y - yhat))
df1<- cbind(df,err);print(df1) #errors from the model and manually calculated 

# extracting y-hats 
yhat <- X %*% bhat

#plot that illustrates y's and estimated y's
mtcars %>% 
  ggplot(aes(wt, mpg))+
...
```{r}
#add a third column to X that is squared
designmatrix <- cbind(constant= rep(1, 32), mtcars$wt, (mtcars$wt^2))

#estimate the quadratic beta using Ordinary Least Squares
bhat_quadratic <- solve(t(designmatrix)%*%designmatrix)%*%t(designmatrix)%*%Y

bhat_quadratic
```

*Little note to self*:
When the x2 positive then the model is convex (happy) and when x2 is negative when the curve is concave (sad).
...
```{r}
#making a lil dataframe with the data I need
designmatrix_tibble <- tibble(constant= rep(1, 32), "x"= mtcars$wt, "x2" = (mtcars$wt^2), y=mtcars$mpg)

#compare the quadratic beta-hat with model's beta-hat
model1 <- lm(y~x + x2, data=designmatrix_tibble)
summary(model1)#its the same beta values as our quadratic betas:)

#estimating y-estimates from the quadratic model
yhat_quadratic <- designmatrix%*%bhat_quadratic

#plot for comparison of y and y-model-estimates
mtcars %>% 
  ggplot(aes(wt, mpg))+
...
From visual inspection the quadratic plot seems to have a better fit. 


2. calculate the sum of squared errors, (show the calculation based on $\epsilon$). Which fit has the lower sum?  
```{r}
RSS_linear <- sum((Y-yhat)^2)
RSS_linear # 278.3219 

yhat
RSS_quadratic <- sum((Y-yhat_quadratic)^2)
RSS_quadratic  #203.7454

```
The quadratic model makes the lowest sum of RSS. 
...
```{r}
#add a four column to X that is squared
designmatrix_2 <- cbind(constant= rep(1, 32), mtcars$wt, (mtcars$wt^2), (mtcars$wt^3))

#estimate the quadratic beta using Ordinary Least Squares
bhat_cubic <- solve(t(designmatrix_2)%*%designmatrix_2)%*%t(designmatrix_2)%*%Y

#put it into a tibble 
designmatrix_tibble2 <- tibble(constant= rep(1, 32), "x"= mtcars$wt, "x2" = (mtcars$wt^2),"x3"=(mtcars$wt^3), y=mtcars$mpg)

#make a cubic model
model2 <- lm(y~x + x2 + x3, data=designmatrix_tibble2)
summary(model2)

#generate y estimates from cubic model
yhat_cubic <- designmatrix_2%*%bhat_cubic

# ------ compare the cubic fit to quadratic fit visually -------# 
...
cubic<- mtcars %>% 
  ggplot(aes(wt, mpg))+
  geom_point(aes(y = yhat_cubic), colour = "orange") +
  geom_point(aes(y = Y), colour = "darkred")

cubic/quadratic

# ------ compare the cubic fit to quadratic fit by RSS-------# 

RSS_quadratic <- sum((Y-yhat_quadratic)^2)
RSS_quadratic  #203.7454

RSS_cubic <- sum((Y-yhat_cubic)^2)
RSS_cubic # 203.6699

#The estimated value of the "cubic" (${\beta}_3$) parameter
coef(model2) # the cubic  (${\beta}_3$) parameter is 0.04593618

```

i. Visually the quadratic and cubic plot looks quite the same.
ii. Looking at the RSS, the quadratic and cubic fit looks pretty much the same. 
iii. The cubic  (${\beta}_3$) parameter is 0.04593618, which is really small, this means that it has very little effect on the fit, so it does not really contribute much to making a better fit. 

*Even though for example, in the exercises, making the model quadratic doesn't actually add extra information (by adding the squared value), but is it still useful because there are cases of non-linear relations in real data?*

*- Lau: Yes, if the data at hands follows a quadratic distribution, it will be useful to model this. The point is that it won't add much if the data can mostly be explained as a linear relationship. So always assess whether or not it is useful to add more terms - informally spoken, only add terms that make a "big" difference in the amount of variance explained*


## Exercise 3
Doing a logistic regression - estimating the probability that a car has automatic transmission (0) or manual transmission (1) based on its weight. Use the variable "am"	 Transmission (0 = automatic, 1 = manual)  
```{r, eval=FALSE}
#make the model
...
#make a plot df from model
plot_df <- broom::augment(model3, type.predict = "response")
head(plot_df)

#plotting the fitted values for the logistic model
log_reg_plot <-
  ggplot(plot_df, aes(x = wt)) +
  geom_line(aes(y = .fitted), color = "blue") +
  labs(x = "weight", y = "am") + geom_point(aes(y = am), alpha = 0.2);print(log_reg_plot)
...
1. plot the fitted values for __logistic.model__:  
    i. what is the relation between the __linear.predictors__ $X \beta$ and the __fitted_values__ of the __logistic.model__ object?
    -  ????????????????

```{r}

#log odds into probability for estimate of intercept  (0)
boot::inv.logit(12.040) # = 0.9999941 == 0.99%

#log odds into probability for estimate of going from 0 to 1
boot::inv.logit(12.040 + (-4.024))  #=0.99967== 0.99%
...
Hej Lau, det her kan jeg/vi ikke finde ud af :)) 
  - Det kunne være rigtig rart med noget hjælp til dette /              gennemgang af opgaven. 
