...
pacman::p_load(tidyverse,dplyr, data.table, vroom, ggplot2, readbulk, lme4,MuMIn)
```
...
i.
The output of the model indicates that "task" is significant for all levels but "quaduplet", therefore task does explain performance for "singles" and "pairs". Though, inspecting the coefficients of the model, it makes great sense that "quadruplet" is negative, which indicates more a decreese in "correct" answers, i.e when complexity increases the amount correct answer decreases. 
...
MuMIn::r.squaredGLMM(model_correct4) # R2c = 0.0.3031205 #best
