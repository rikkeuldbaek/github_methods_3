...
In exercise 2.1 we use logistic regression with in-sample validation, which means that we train and test our model on the same train and test set throughout the entire analysis. This means the model only becomes really good at predicting that specific test set, and will perform worse when predicting unseen new data.

In exercise 2.2 we use cross-validation with 5 folds, which means that we split our whole data into training and test set through 5 iterations (5 folds) and get a performance measure (classification accuracy score) for each fold. Then we take the mean of the performance measure from the 5 folds in order to evaluate our model. This way the model becomes better at predicting unseen new data. 
