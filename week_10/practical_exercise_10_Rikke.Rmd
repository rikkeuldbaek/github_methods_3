---
title: "Assignment 4, Methods 3, 2021, autumn semester"
author: "Rikke Uldbæk"
...
#as we see here, some of the first priciple components are huge compared to the last ones
...
The increase of classification accuracy in percentage points is 8%. 
...
pca_16 = PCA(n_components=16) #number of components 
cv_scores_pca_16 =  [] #make an empty list

for i in range(251):
  transformed_reduced_data = scaler.fit_transform(data_equal[:,:,i]) 
  # transform the data first
  PCA_16 = pca_16.fit_transform(transformed_reduced_data)
  logReg.fit(PCA_16, y) 
  #loop and fit through the transformed data
  pca_scores_1= cross_val_score(logReg, PCA_16, y, cv=5) 
  #calculate cross validation scores
  cv_scores_pca_16.append(np.mean(pca_scores_1)) 
...
plt.plot(times,cv_scores_pca_16)
...
np.max(cv_scores_pca_16) #0.3559493670886076
...
The main difference between the two analysis, when focusing on the time interval 0ms - 400 ms, is the number of principal components used when classifying. According to the plot 3, it seems like the first 16 principle components (indexed from 0) result in the greatest classification accuracy of 36%, while the total number of principle components (102) performs only a little worse with a maximal classification accuracy of 34%. It makes sense, that the classification score for the reduced data is better at time point 248 as we found in the earlier assignment (Assignment 3) that this is the exact time point where the average peak of magnetic activity was highest. 

Even though the 16 principle components seem to perform better, it is in fact only a slightly better performance than the 102 principle components. 
